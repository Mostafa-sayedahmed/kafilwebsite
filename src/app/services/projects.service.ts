...
import { collection, Firestore,query, where, getDocs, addDoc, docData, doc } from '@angular/fire/firestore';
...
// ==========================================================================================
  ...
  getCategoryProject(){
      ...
      return collectionData(category, {idField:'id'} ) as Observable<categoryprojects[]> ;
  }
  // *************************
    // get Category By ID
  async getCategoryByID(catID:string){
    const q = query(collection(this.firestore, "projects"), where("catId", "==", catID));
  ...
  async getCategoryByopenStatus(){
    const q = query(collection(this.firestore, "projects"), where("open", "==", true));
    const querySnapshot = await getDocs(q);
  ...
  getprojectsByDeliverytime(){
    ...
    return collectionData(DeliveryTime, {idField:'id'} ) as Observable<categoryprojects[]> ;
}
